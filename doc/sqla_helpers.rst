...
For access method from model, models need to be declare as bellow:
...
To avoid mixin uses, :class:`sqla_helpers.base_model.BaseModel` class can be use as `cls` parameter in :func:`declarative_base`
function. 
...
Register a session maker can be dangerous. Because, technically, we change dynamically a classemethod. To prevent errors, an exception , :exc:`sqla_helpers.base_model.SessionMakerExists`, is raised if a session maker is already registered.

But sometimes, may be you need to change it while application is running. So, you can force an new record even a session maker is already registered
